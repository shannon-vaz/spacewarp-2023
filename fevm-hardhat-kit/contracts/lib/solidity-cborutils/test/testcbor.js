...
contract("CBOR", function (accounts) {
    ...
    it("returns > 8 byte int as bytes", async function () {
        var test = await TestCBOR.new()
        var result = cbor.decodeFirstSync(
            new Buffer.from((await test.getTestDataBigInt()).slice(2), "hex")
        )
    ...
    it("returns > 8 byte uint as bytes", async function () {
        var test = await TestCBOR.new()
        var result = cbor.decodeFirstSync(
            new Buffer.from((await test.getTestDataBigUint()).slice(2), "hex")
        )
        expect(result.map((x) => x.toFixed())).to.deep.equal([
            "0",
            "1",
    ...
    it("returns definite-length encoded array", async function () {
        ...
        var result = cbor.decodeFirstSync(
            new Buffer.from((await test.getTestDataDefiniteLengthArray()).slice(2), "hex")
        )
    ...
    it("returns definite-length encoded map", async function () {
        var test = await TestCBOR.new()
        var result = cbor.decodeFirstSync(
            new Buffer.from((await test.getTestDataDefiniteLengthMap()).slice(2), "hex")
        )
    ...
    it("reverts for invalid CBOR", async function () {
        var test = await TestCBOR.new()
        await truffleAssert.reverts(test.getTestDataInvalidCBOR(), "Invalid CBOR")
    })
})
