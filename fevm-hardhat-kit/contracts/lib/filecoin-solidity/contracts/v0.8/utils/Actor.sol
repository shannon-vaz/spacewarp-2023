...
library HyperActor {
    ...
    function call_inner(
        ...
        ) = call_actor_address(convert(method), amount, DEFAULT_FLAG, codec, params, filAddress);
    ...
    function call_inner(
        ...
        assembly {
            ...
            for {
                ...
                mstore(
                    add(input, add(start_index, offset)),
                    mload(add(raw_request, add(0x20, offset)))
                )
            }
            ...
            for {
                ...
                mstore(
                    add(input, add(start_index, offset)),
                    mload(add(actor_code, add(0x20, offset)))
                )
            }
